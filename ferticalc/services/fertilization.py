...
class IndividualSelection(str, Enum):
    USER = "user"
    SOFTWARE = "software"
...
_FOSFATADO_POR_NORMALIZED = {
    _normalize_name(item.nome): item for item in _FOSFATADOS_SEQ
}
...
_POTASSICO_POR_NORMALIZED = {
    _normalize_name(item.nome): item for item in _POTASSICOS_SEQ
}
...
NITROGENADOS: Dict[str, Fertilizante] = {
    item.codigo: item for item in _NITROGENADOS_SEQ
}
_NITROGENADO_POR_NOME = {item.nome: item for item in _NITROGENADOS_SEQ}
_NITROGENADO_POR_NORMALIZED = {
    _normalize_name(item.nome): item for item in _NITROGENADOS_SEQ
}
...
def format_formulated_name(grade: Dict[str, float], default: str = "Formulado") -> str:
    try:
        n = grade.get("N", 0.0)
        p = grade.get("P2O5", 0.0)
        k = grade.get("K2O", 0.0)
    except AttributeError:
        return default
    return f"N {n:g} - P {p:g} - K {k:g}"


def _adicionar_produto(
    destino: List[Tuple[str, float]], nome: str, quantidade: float, minimo: float = 1e-6
) -> None:
...
def _subtrair_fornecido(requerido: float, quantidade_kg: float, fracao: float) -> float:
    ...
    *,
    ...
    if p_restante > 0:
        ...
        if fert_p.p2o5 <= 0:
            ...
            alertas.append(
                "Selecione um fertilizante fosfatado valido para complementar o P2O5."
            )
        else:
    ...
    if k_restante > 0:
        ...
        if fert_k.k2o <= 0:
            ...
            alertas.append(
                "Selecione um fertilizante potassico valido para complementar o K2O."
            )
        else:
    ...
    if n_restante > 0:
        ...
        if fert_n.n <= 0:
            ...
            alertas.append(
                "Selecione um fertilizante nitrogenado valido para complementar o N."
            )
        else:
...
def calcular_individual_usuario(
    ...
    if p_req > 0:
        if fert_p is None or fert_p.p2o5 <= 0:
            ...
            if fert_p is None:
                alertas.append(
                    "Selecione um fertilizante fosfatado para atender ao P2O5."
                )
            else:
                alertas.append(
                    f"O fertilizante '{fert_p.nome}' nao fornece P2O5 suficiente."
                )
        else:
    ...
    if k_req > 0:
        if fert_k is None or fert_k.k2o <= 0:
            ...
            if fert_k is None:
                alertas.append(
                    "Selecione um fertilizante potassico para atender ao K2O."
                )
            else:
                alertas.append(
                    f"O fertilizante '{fert_k.nome}' nao fornece K2O suficiente."
                )
        else:
    ...
    if n_req > 0:
        if fert_n is None or fert_n.n <= 0:
            ...
            if fert_n is None:
                alertas.append(
                    "Selecione um fertilizante nitrogenado para atender ao N."
                )
            else:
                alertas.append(
                    f"O fertilizante '{fert_n.nome}' nao fornece N suficiente."
                )
        else:
            kg_n = n_req / fert_n.n
            _adicionar_produto(produtos, fert_n.nome, kg_n)
...
def _make_demanda(requirement: FertilizerRequirement) -> Dict[str, float]:
    return {
        ...
        "S": max(requirement.s_kg_ha, 0.0),
        "Mo": max(requirement.mo_kg_ha, 0.0),
    }
    ...
    *,
    ...
    try:
        if mode is FertilizationMode.FORMULATED:
            ...
            nome = formulated_name or format_formulated_name(
                formulated_grade, "Formulado personalizado"
            )
            ...
            nome = formulated_name or format_formulated_name(
                formulated_grade, "Formulado selecionado"
            )
            resultado = calcular_formulado(
                demanda,
                formulated_grade,
...
def format_products(
    ...
    *,
    ...
    def _format_quantity(
        kg_value: float, area: float, por_area_local: bool, unit_local: str
    ) -> str:
